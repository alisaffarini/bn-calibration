...
def run_experiment():
    ...
    # Compute final statistics - FIXED SECTION
    print(f"\n{'='*50}")
    print("FINAL RESULTS")
    print('='*50)
    ...
    for method in results:
        ...
        per_seed_results = {
            ...
            'converged': [int(c) for c in results[method]['converged']]  # Convert bool to int
        }
    ...
    signal_detected = (np.mean(results['cabn']['ece']) < np.mean(results['standard_bn']['ece']) and ece_improvement_percent > 2)
    ...
    final_results['signal_detected'] = int(signal_detected)  # Convert bool to int
