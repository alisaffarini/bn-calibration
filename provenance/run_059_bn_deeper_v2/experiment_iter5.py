...
def run_fast_experiment(seed):
    ...
    return {
        ...
        'non_monotonic': bool(is_non_monotonic),  # Convert to native Python bool
        'min_acc_alpha': float(alphas[min_idx]),  # Convert to native Python float
        'base_accuracy': float(base_acc)  # Convert to native Python float
    }
...
def main():
    ...
    output = {
        'per_seed_results': all_results,  # Already has native Python types now
