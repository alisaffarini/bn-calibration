    ...
    with torch.no_grad():
        for inputs, labels in dataloader:
            ...
            for name, layer in bn_layers:
                ...
                # Get the proper shape for statistics
                if len(act.shape) == 4:  # Conv layers (N, C, H, W)
                    N, C, H, W = act.shape
                    act_reshaped = act.permute(0, 2, 3, 1).reshape(-1, C)  # (N*H*W, C)
                elif len(act.shape) == 2:  # FC layers (N, C)
                    act_reshaped = act
                ...
                if class_conditional:
                    ...
                    for c in range(num_classes):
                        mask = labels == c
                        if mask.sum() > 0:
                            # For conv layers, we need to expand the mask to match spatial dimensions
                            if len(act.shape) == 4:
                                mask_expanded = mask.view(-1, 1, 1, 1).expand_as(act)
                                mask_flat = mask_expanded.permute(0, 2, 3, 1).reshape(-1, C)
                                class_act = act_reshaped[mask_flat[:, 0]]  # All channels have same mask
                            else:
                                class_act = act_reshaped[mask]
                            
                            if class_act.shape[0] > 0:  # Make sure we have samples
                    ...
                    if stats[name]['sum'] is None:
                        stats[name]['sum'] = act_reshaped.sum(0)
                        stats[name]['sq_sum'] = (act_reshaped ** 2).sum(0)
                        stats[name]['count'] = act_reshaped.shape[0]
                    else:
                        stats[name]['sum'] += act_reshaped.sum(0)
                        stats[name]['sq_sum'] += (act_reshaped ** 2).sum(0)
                        stats[name]['count'] += act_reshaped.shape[0]
...
class SimpleResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10):
        ...
        self.linear = nn.Linear(32 * 4 * 4, num_classes)  # Fixed: 32 channels * 4x4 spatial
    ...
    def forward(self, x):
        ...
        out = F.avg_pool2d(out, 4)  # (N, 32, 4, 4) - Fixed: use 4 instead of 8
        out = out.view(out.size(0), -1)  # (N, 32*4*4)
        out = self.linear(out)
        return out
...
def run_interpolation_experiment(seed):
    ...
    for alpha in alphas:
        ...
        for name, layer in extract_bn_layers(model):
            ...
            if all_means:  # Make sure we have stats
        ...
        for name in global_stats:
            if name in avg_class_stats:
                interp_stats[name] = interpolate_stats(global_stats[name], avg_class_stats[name], alpha)
            else:
                interp_stats[name] = global_stats[name]  # Use global if no class stats
