...
def train_model(model, train_loader, val_loader, device, epochs=100, patience=15, lr=0.1, is_standard=False):
...
def run_experiment(seed, use_full_data=True):
    ...
    converged, val_acc = train_model(model, train_loader, val_loader, device, 
                                    epochs=100 if use_full_data else 30, patience=15)
    ...
    std_converged, std_val_acc = train_model(standard_model, train_loader, val_loader, device, 
                                           epochs=100 if use_full_data else 30, patience=15, is_standard=True)
