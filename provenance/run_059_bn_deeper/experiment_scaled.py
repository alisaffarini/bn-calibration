...
def generate_challenging_cifar(num_train=5000, num_test=1000, num_classes=10):
    ...
    def generate_class_data(n_samples, class_id, num_classes):
        ...
        for i in range(3):  # 3 patterns per class
            ...
            # Add pattern with some randomness
            pattern_strength = 1.5 + torch.randn(n_samples, 1, 1, 1) * 0.3
...
# ==================== MAIN EXPERIMENT ====================
def run_single_seed(seed, dataset='challenging'):
    ...
    try:
        ...
        for epoch in range(100):
            ...
            if patience_counter >= 15 and epoch >= 30:
...
def analyze_results(all_results):
    """Analyze results across all seeds with publication-quality statistics."""
    ...
    for layer_idx in range(num_tested_layers):
        ...
        if len(layer_data) >= 3:
            ...
            if len(mean_eces) >= 11:  # Full alpha sweep
                ...
                # Compute gradients at layer
                grad_importance = np.mean([r['gradient_importance'].get(layer_idx, 0) 
                                         for r in all_results if 'gradient_importance' in r])
    ...
    if len(bn_eces) > 1 and len(best_interpolation_eces) > 1:
        # Paired t-test for BN vs interpolation
    ...
    bn_ece_ci = bootstrap_ci(bn_eces) if len(bn_eces) > 1 else (bn_eces[0], bn_eces[0])
    interp_ece_ci = bootstrap_ci(best_interpolation_eces) if len(best_interpolation_eces) > 1 else (best_interpolation_eces[0], best_interpolation_eces[0])
    
    # Count U-curves
    u_curve_count = sum(1 for v in u_curve_analysis.values() if v['is_u_curve'])
    
    # Gradient-importance correlation
    grad_alpha_correlation = []
    for layer_idx, analysis in u_curve_analysis.items():
        if 'gradient_importance' in analysis and 'min_alpha' in analysis:
            grad_alpha_correlation.append({
                'layer': layer_idx,
                'gradient': analysis['gradient_importance'],
                'optimal_alpha': analysis['min_alpha']
            })
    ...
    for i, result in enumerate(all_results):
        seed_data = {
            'random_acc': result.get('random_baseline', {}).get('test_acc', 0.0),
            'random_ece': result.get('random_baseline', {}).get('test_ece', 1.0),
            'simple_acc': result.get('simple_baseline', {}).get('test_acc', 0.0),
            'simple_ece': result.get('simple_baseline', {}).get('test_ece', 1.0),
            'bn_acc': result.get('bn_baseline', {}).get('test_acc', 0.0),
            'bn_ece': result.get('bn_baseline', {}).get('test_ece', 1.0),
            'temp_ece': result.get('temperature_scaling', {}).get('test_ece', 1.0),
            'converged': result.get('bn_baseline', {}).get('converged', False)
        }
        ...
        if 'interpolation' in result and len(result['interpolation']) > 0:
            ...
            for layer in result['interpolation'].values():
                for point in layer:
                    if point['ece'] < best_ece:
                        best_ece = point['ece']
            seed_data['best_interp_ece'] = best_ece
    ...
    final_results = {
        ...
        'main_result': {
            ...
            'gradient_alpha_correlation': grad_alpha_correlation
        },
        'ablations': {
            'temperature_scaling': {
...
def main():
    ...
    first_result = run_single_seed(0)
    ...
    bn_acc = first_result['bn_baseline']['test_acc']
    bn_ece = first_result['bn_baseline']['test_ece']
    simple_acc = first_result['simple_baseline']['test_acc']
    simple_ece = first_result['simple_baseline']['test_ece']
    random_acc = first_result['random_baseline']['test_acc']
