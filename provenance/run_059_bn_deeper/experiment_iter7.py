...
def generate_challenging_cifar(num_train=5000, num_test=1000, num_classes=10):
    ...
    def generate_class_data(n_samples, class_id, num_classes):
        ...
        for i in range(3):  # 3 patterns per class
            ...
            pattern_strength = pattern_strength.view(n_samples, 1, 1)
...
def analyze_results(all_results):
    ...
    for layer_idx in range(num_tested_layers):
        ...
        if len(layer_data) >= 3:
            ...
            if len(mean_eces) >= 5:  # Need enough points
                ...
                grad_importance = np.mean([r.get('gradient_importance', {}).get(layer_idx, 0) 
    ...
    if not best_interpolation_eces:
        best_interpolation_eces = bn_eces.copy() if bn_eces else [1.0]
    ...
    final_results = {
        ...
        'main_result': {
            ...
            'total_tested_layers': len(u_curve_analysis),
