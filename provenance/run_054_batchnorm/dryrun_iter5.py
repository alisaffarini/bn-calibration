...
def sanity_check_mutual_information():
    ...
    # Simulate BN stats where each class has distinct statistics
    bn_stats_perfect = []
    labels_perfect = []
    ...
    labels_random = np.random.randint(0, n_classes, n_samples)
    ...
    assert mi_perfect > 0.5, f"Perfect MI too low: {mi_perfect}"
    assert mi_random < 0.1, f"Random MI too high: {mi_random}"
    assert mi_random < mi_partial < mi_perfect, f"MI ordering wrong"
    ...
    for _ in range(10):  # 10 batches per class
        for class_idx in range(n_classes):
    ...
    for batch_idx in range(100):
        ...
        batch_stats += np.random.normal(0, 0.2, n_features)
    ...
    assert mi_single_class_batches > mi_mixed_batches * 2, \
        f"Single-class MI should be much higher: {mi_single_class_batches:.3f} vs {mi_mixed_batches:.3f}"
...
def train_model_convergence(model, train_loader, val_loader, device, max_epochs=3, patience=2, lr=0.001):
...
def run_single_seed_experiment(seed, model_type='bn', sampling_strategy='random', 
    ...
    val_acc, converged = train_model_convergence(
        ...
        max_epochs=max_epochs, patience=2, lr=lr
    )
