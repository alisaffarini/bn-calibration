...
def create_harder_dataset(n_samples=10000, n_features=32, n_classes=10, difficulty='very_hard', split='train'):
    """
...
################################################################################
# BN STATS TRACKER (PRESERVED + ENHANCED)
################################################################################
...
################################################################################
# MODEL DEFINITIONS (PRESERVED FROM WORKING VERSION)
################################################################################
...
class ModelNoBN(nn.Module):
    """Model without normalization (baseline)."""
    def __init__(self, input_dim=32, hidden_dims=[128, 256, 128], n_classes=10):
        super().__init__()
...
def train_model_convergence(model, train_loader, val_loader, device, sampling_strategy, 
    ...
    for epoch in range(max_epochs):
        ...
        # Progress every 10 epochs or early
...
def run_single_seed_experiment(seed, model_type='bn', sampling_strategy='random', 
    ...
    if dataset == 'synthetic':
        ...
        # Real dataset loading (e.g., CIFAR-10)
    ...
    if model_type == 'bn' and hasattr(model, 'stats_tracker'):
        ...
        if mi_info.get('n_single_batches', 0) > 10 and mi_info.get('n_mixed_batches', 0) > 10:
            ...
            if single_norms and mixed_norms:
                threshold = (np.median(single_norms) + np.median(mixed_norms)) / 2
...
def main():
    ...
    # 2. Comprehensive baselines (5 seeds)
    ...
    for seed in range(5):
        ...
        result = run_single_seed_experiment(
            ...
            hidden_dims=[512], max_epochs=60,
            difficulty='very_hard', experiment_name='ablation_shallow'
        )
        all_results.append(result)
        ...
        result = run_single_seed_experiment(
            seed=seed, model_type='bn', sampling_strategy='single_class',
            hidden_dims=[128, 256, 512, 256, 128], max_epochs=60,
            difficulty='very_hard', experiment_name='ablation_deep'
        )
        all_results.append(result)
    
    # Ablation 2: BN momentum (3 seeds)
    ...
    for seed in range(3):
        for momentum in [0.01, 0.5, 0.9]:
            result = run_single_seed_experiment(
                seed=seed, model_type='bn', sampling_strategy='single_class',
                hidden_dims=[256, 512, 256], max_epochs=60,
                bn_momentum=momentum, difficulty='very_hard',
                experiment_name=f'ablation_momentum_{momentum}'
            )
            all_results.append(result)
    
    # Ablation 3: Dataset difficulty (3 seeds)
    print("\n3c. Dataset difficulty ablation")
    for seed in range(3):
        for diff in ['hard', 'very_hard']:
            result = run_single_seed_experiment(
                seed=seed, model_type='bn', sampling_strategy='single_class',
                hidden_dims=[256, 512, 256], max_epochs=60,
                difficulty=diff, experiment_name=f'ablation_difficulty_{diff}'
            )
            all_results.append(result)
    
    # Optional: Real dataset validation
    """
    print("\n4. Real dataset validation (2 seeds)")
    for seed in range(2):
        for sampling in ['random', 'single_class']:
            result = run_single_seed_experiment(
                seed=seed, model_type='bn', sampling_strategy=sampling,
                hidden_dims=[512, 512], max_epochs=40,
    ...
    print("\nMain Results (mean ± std, [95% CI]):")
    print("-" * 140)
    print(f"{'Method':<30} {'Test Acc':<35} {'MI':<35} {'Batch Type Pred':<25} {'Conv%'}")
    print("-" * 140)
    ...
    for method, metrics in sorted(summary.items()):
        if metrics['test_acc']:
            ...
            print(f"{method:<30} {acc_mean:.3f}±{acc_std:.3f} [{acc_lower:.3f},{acc_upper:.3f}]   "
                  f"{mi_mean:.3f}±{mi_std:.3f} [{mi_lower:.3f},{mi_upper:.3f}]   "
    ...
    if 'bn_random' in summary_dict and 'bn_single_class' in summary_dict:
        ...
        # Estimate class-specific variance ratio
        ...
        print(f"Estimated class-specific variance ratio: {variance_ratio:.1%}")
    ...
    for seed in range(n_seeds):
        ...
        if random_results and single_results:
            ...
            bn_random_acc.append(random_results[0]['test_acc'])
            bn_single_acc.append(single_results[0]['test_acc'])
    
    if len(bn_random_mi) >= 2:
        # MI comparison
        t_stat_mi, p_val_mi = ttest_rel(bn_single_mi, bn_random_mi)
        print(f"BN single vs random MI (paired t-test): t={t_stat_mi:.3f}, p={p_val_mi:.6f}")
        
        # Accuracy comparison
        t_stat_acc, p_val_acc = ttest_rel(bn_single_acc, bn_random_acc)
        print(f"BN single vs random Acc (paired t-test): t={t_stat_acc:.3f}, p={p_val_acc:.6f}")
        
        # Effect size (Cohen's d)
        diff_mi = np.array(bn_single_mi) - np.array(bn_random_mi)
        cohens_d = np.mean(diff_mi) / (np.std(diff_mi) + 1e-8)
        print(f"Effect size for MI (Cohen's d): {cohens_d:.3f}")
    ...
    for ablation_name, results_list in sorted(ablation_results.items()):
        if results_list:
            acc_vals = [r['test_acc'] for r in results_list]
            mi_vals = [r['avg_mi'] for r in results_list]
            print(f"{ablation_name:<35} Acc: {np.mean(acc_vals):.3f}±{np.std(acc_vals):.3f}, "
                  f"MI: {np.mean(mi_vals):.3f}±{np.std(mi_vals):.3f}")
    
    # Hypothesis testing
    print("\nHypothesis Testing:")
    print("-" * 80)
    
    # H1: BN contains class-specific information
    ...
    # H2: Layer-wise pattern
    if len(layer_mis_ordered) >= 3:
        # Check if early layers have higher MI than later layers
        early_avg = np.mean(layer_mis_ordered[:len(layer_mis_ordered)//2])
        late_avg = np.mean(layer_mis_ordered[len(layer_mis_ordered)//2:])
        h2_supported = early_avg > late_avg
        print(f"H2 (early layers encode more class info): {'SUPPORTED' if h2_supported else 'NOT SUPPORTED'} "
              f"(Early: {early_avg:.3f}, Late: {late_avg:.3f})")
    else:
        h2_supported = False
        print("H2: INSUFFICIENT DATA")
    
    # H3: Sampling strategy affects MI
    h3_supported = 'p_val_mi' in locals() and p_val_mi < 0.001
    ...
    final_results = {
        ...
        'statistical_tests': {
            'bn_single_vs_random_mi_pval': float(p_val_mi) if 'p_val_mi' in locals() else None,
            'bn_single_vs_random_acc_pval': float(p_val_acc) if 'p_val_acc' in locals() else None,
            'effect_size_cohens_d': float(cohens_d) if 'cohens_d' in locals() else None,
            'mi_lift': float(mi_lift) if 'mi_lift' in locals() else None
        },
        'hypotheses': {
            'h1_bn_contains_class_info': bool(h1_supported),
            'h2_early_layers_higher_mi': bool(h2_supported),
