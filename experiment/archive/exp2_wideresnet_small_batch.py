#!/usr/bin/env python3
"""
Experiment 2: WideResNet-28-10 on CIFAR-10.
3 seeds (42-44), 20 epochs, 4 BN conditions.
"""
...
def train_model(model, train_loader, test_loader, epochs, seed, label=""):
    ...
    num_bn = len(get_bn_layers(model))
    print(f"  Model has {num_bn} BatchNorm layers, {sum(p.numel() for p in model.parameters())/1e6:.1f}M params")
...
def main():
    ...
    print("=" * 70)
    print("EXPERIMENT 2: WideResNet-28-10 on CIFAR-10 (3 seeds)")
    print("=" * 70)

    train_loader, test_loader, cl_train, cl_test, nc = get_cifar10_data()
    all_results = []

    for i, seed in enumerate(SEEDS):
        t0 = time.time()
        print(f"\n--- WideResNet Seed {seed} ({i+1}/{len(SEEDS)}) ---")

        torch.manual_seed(seed)
        model = WideResNet(depth=28, widen_factor=10, dropout_rate=0.3, num_classes=nc).to(DEVICE)
        model = train_model(model, train_loader, test_loader, EPOCHS, seed, f"WRN s{seed}")
    ...
    out_path = OUTPUT_DIR / 'results_wideresnet.json'
    with open(out_path, 'w') as f:
        json.dump(results, f, indent=2)

    print("\n" + "=" * 70)
    print(f"WideResNet-28-10 RESULTS")
    print("=" * 70)
