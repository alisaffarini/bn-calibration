#!/usr/bin/env python3
"""
Experiment 3: Tiny-ImageNet (200 classes, 64x64).
SmallResNet adapted for 64x64, 3 seeds (42-44), 25 epochs, 4 BN conditions.
"""
...
# ─────────────────────────────────────────────
# Model: SmallResNet adapted for 64x64
# ─────────────────────────────────────────────
...
class SmallResNet64(nn.Module):
    """SmallResNet adapted for 64x64 input — adds extra layer for more downsampling."""
    def __init__(self, num_classes=200):
        super().__init__()
        self.in_planes = 32
        self.conv1 = nn.Conv2d(3, 32, 3, stride=1, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(32)
        self.layer1 = self._make_layer(32, 2, stride=1)   # 64x64
        self.layer2 = self._make_layer(64, 2, stride=2)   # 32x32
        self.layer3 = self._make_layer(128, 2, stride=2)  # 16x16
        self.layer4 = self._make_layer(256, 2, stride=2)  # 8x8
        self.layer5 = self._make_layer(512, 2, stride=2)  # 4x4
...
def compute_class_conditional_bn_stats(model, dataloader_by_class, num_classes):
    ...
    for c in range(num_classes):
        ...
        if (c + 1) % 50 == 0:
            print(f"      BN stats computed for {c+1}/{num_classes} classes")
...
class TinyImageNetDataset(Dataset):
    def __init__(self, root_dir, split='train', transform=None):
        ...
        # Build class-to-idx mapping from wnids.txt
        ...
        if split == 'train':
            for wnid in self.wnids:
                class_dir = self.root_dir / 'train' / wnid / 'images'
                if class_dir.exists():
                    for img_file in sorted(class_dir.glob('*.JPEG')):
                        self.images.append(str(img_file))
                        self.targets.append(self.class_to_idx[wnid])
        elif split == 'val':
            val_annotations = self.root_dir / 'val' / 'val_annotations.txt'
            with open(val_annotations, 'r') as f:
                for line in f.readlines():
                    parts = line.strip().split('\t')
                    img_name = parts[0]
                    wnid = parts[1]
                    img_path = self.root_dir / 'val' / 'images' / img_name
                    if img_path.exists() and wnid in self.class_to_idx:
                        self.images.append(str(img_path))
                        self.targets.append(self.class_to_idx[wnid])

        print(f"  TinyImageNet {split}: {len(self.images)} images, {len(set(self.targets))} classes")
...
def download_tiny_imagenet():
    ...
    if tiny_dir.exists() and (tiny_dir / 'wnids.txt').exists():
    ...
    if not zip_path.exists():
        ...
        try:
            ...
            print(f"  Download complete ({zip_path.stat().st_size / 1e6:.0f} MB)")
...
def get_tinyimagenet_data():
    """Load Tiny-ImageNet dataset."""
    tiny_dir = download_tiny_imagenet()
    if tiny_dir is None:
        return None
    ...
    trainset = TinyImageNetDataset(tiny_dir, split='train', transform=transform_train)
    testset = TinyImageNetDataset(tiny_dir, split='val', transform=transform_test)
    trainset_clean = TinyImageNetDataset(tiny_dir, split='train', transform=transform_test)
...
def linear_probe(features, labels, num_classes=200):
    """Linear probe using LogisticRegression."""
    n = len(labels)
...
def run_standard_eval(model, class_loaders_train, class_loaders_test, num_classes, seed):
    ...
    print(f"      global: acc={conditions['global']['mean_acc']:.4f}, probe={probe:.4f}")
    ...
    print(f"      same_class: acc={conditions['same_class']['mean_acc']:.4f}, probe={probe:.4f}")
    ...
    print(f"      wrong_class: acc={conditions['wrong_class']['mean_acc']:.4f}, probe={probe:.4f}")
    ...
    print(f"      random_class: acc={conditions['random_class']['mean_acc']:.4f}, probe={probe:.4f}")
...
def main():
    ...
    print("=" * 70)
    print("EXPERIMENT 3: SmallResNet64 on Tiny-ImageNet (200 classes, 3 seeds)")
    print("=" * 70)

    data = get_tinyimagenet_data()
    if data is None:
        print("SKIPPING: Tiny-ImageNet download failed.")
        results = {'error': 'Download failed', 'skipped': True}
        out_path = OUTPUT_DIR / 'results_tinyimagenet.json'
        with open(out_path, 'w') as f:
            json.dump(results, f, indent=2)
        return

    train_loader, test_loader, cl_train, cl_test = data
    all_results = []

    for i, seed in enumerate(SEEDS):
        t0 = time.time()
        print(f"\n--- Tiny-ImageNet Seed {seed} ({i+1}/{len(SEEDS)}) ---")

        torch.manual_seed(seed)
        model = SmallResNet64(num_classes=NUM_CLASSES).to(DEVICE)
        num_params = sum(p.numel() for p in model.parameters()) / 1e6
        print(f"  Model: SmallResNet64, {num_params:.1f}M params, {len(get_bn_layers(model))} BN layers")

        model = train_model(model, train_loader, test_loader, EPOCHS, seed, f"TinyIN s{seed}")

        conds = run_standard_eval(model, cl_train, cl_test, NUM_CLASSES, seed)
        conds['seed'] = seed
    ...
    results = {
        'config': {
            'seeds': SEEDS,
            'epochs': EPOCHS,
            'model': 'SmallResNet64 (5-stage, adapted for 64x64)',
